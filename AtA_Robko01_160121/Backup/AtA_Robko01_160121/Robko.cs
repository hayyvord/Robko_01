...
namespace AtA_Robko01_160121
{
    class Robko
    {
        ...
        public void CW(int joint, int steps)
        {
            string command = string.Format("?D{0}+{1:D3}", joint,Varibles.CW, steps, 
                steps);
            this.SendRequest(command);
        }

        public void CWW(int joint, int steps)
        {
            string command = string.Format("?D{0}{1}{2:D3}",joint,Varibles.CWW, steps);
            this.SendRequest(command);
        }

        public void UpDown(string direction ,int steps) 
        {
            string command = string.Format("?M{0}{1:D3}", direction, steps);
            this.SendRequest(command);
        }
        
        public void Speed(int speed)
        {
            string command = string.Format("?S{0:D3}", speed);
            this.SendRequest(command);
        }
